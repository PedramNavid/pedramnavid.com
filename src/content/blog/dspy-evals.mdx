...
In this post I wlll show how you can evaluate and automatically optimize LLM performance on the NYT Connections puzzle using DSPy, taking a simple prompt signature to a fully optimized one with improvements of up to 21% against the baseline.

## Results

Here are the results of the various models I evaluated, both before and after optimization, as long with the duration and total cost. 
...
## Evalulations in DSPy

I struggled to understand the discourse around evals for a long time. Here's my basic attempt at explaining what an eval is and why we do them.

I have some task that I would like to automate using an LLM. The goal is to have the LLM perform some type of prediction. The natural question is where the result of this prediction is any good. We might also ask questions like "how do different models perform on this task?" and "what are the cost/benefit tradeoffs?" or even "can we change parameters to improve performance?"

If this is starting to sound like old fashioned data science, I have bad news for you: it is.
...
If this hasn't scared you away yet, then the rest of this post will break down what this looks like in the context of DSPy. 
...
The full code for this project is on [GitHub](https://github.com/pedramnavid/dspy-connections), which provides the full implementation. In this post, I'll focus on the LLM code rather than the state and puzzle logic.
...
- `rules`: The essential game rules provided to all models can be found in the repo.
...
The model returns a guess prediction, and the application logic is processed in the `forward` method.
...
We could consider other metrics, such as an efficiency (how many attempts it took to solve the puzzle) or factor in things like difficulty, but simplicity is best. We'll validate that the response has given us a well-formed success attribute, and return it if so.
...
An `Example` is a simple object containing an input and, usually, an expected output. In our case, the input is a `Puzzle` object, and the success metric is a state in the puzzle itself, so we don't need to provide an expected output. However, an example will often include a label.

It's important to use the `with_inputs` method on the Example. This is not always documented in the DSPy docs, but without it your evaluation will not work.
...
A side note, if we didn't have a state object, we might do something like `dspy.Example(puzzle=puzzle, answer=answer).with_inputs("guess").` That is the typical Example pattern you'll see in the wild.
...
If those durations seem low, DSPy caches results, which is a nice feature to help avoid costly reruns. Do note that if you start changing some of the state logic though, the cache will not re-run and you might have to manually wipe the disk cache, usually in `~/.dspy_cache`
...
We've already convered running evaluations, now we can run an optimizer. Thankfully in DSPy this is fairly straightforward:
...
I use the `light` auto settings to quickly pick sensible defaults for the optimization process. Luckily, we can also use threads to speed up the process. You can also define a teacher and a prompt model, they are used by MIPRO to generate new instructions and few-shot examples. I kept these consistent across all models to keep the comparison fair. 

Compilation finds the best parameters and prompts through optimization and then saves the results into a compiled model that you can use just like your original module.
...
DSPy has decent integration with MLFlow for experiment tracking. You'll see some helper functions I used to track metrics in the repo. MLFlow also ships with an autolog feature that automatically logs traces and runs, although I did find that can overwhelm the MLFlow server with too many traces, especially if running against a cached prompt through DSPy, or using 32 threads like I did.

The overall experience with MLFlow was a bit buggy. Since running too many threads overloaded the SQLite server that MLFlow uses, I moved to Postgres, but that didn't solve all the issues either. There were various errors about failing to write traces or save models that I didn't feel like debugging for this project.
...
Here's an example of a prompt from grok-3-mini, which saw a 21% improvement from a 32% success rate up to 53% after optimization. Remember, the optimizer, not me, created this prompt. And every model will have a different prompt.
...
What's interesting is seeing how the prompt was optimized. It added phrases like 'You are a master puzzle solver', but also gave hints such as 'look for obvious thematic connections' and 'consider multiple meaning of each word'. These were learned through analysis of both successful and failed predictions on the unoptimized model.


## Final Thoughts

The improvements were pretty dramatic. A 21 percent absolute increase for grok 3 was the most dramatic result and brought it inline with an unoptimized Sonnet 4 model, although the run time was much longer, but the cost much better. GPT5-mini was easily the best model before optimization, and hard to beat after. At only $9.65 it was extremely cost effective too. 
